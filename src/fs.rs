...
impl Fs {
    ...
    /// Resolve a guest path to a host filesystem path. This only works for
    /// files backed by real host paths (not IPA-embedded or resource files).
    /// If the file doesn't exist yet but the parent directory is writable,
    /// the expected host path is returned.
    pub fn host_path_for(&mut self, path: &GuestPath) -> Option<PathBuf> {
        if let Some(node) = self.lookup_node(path) {
            match node {
                FsNode::File {
                    location: FileLocation::Path(host_path),
                    ..
        ...
        // File doesn't exist yet; compute where it would be created.
        let (parent_node, filename) =
            self.lookup_parent_node(path)?;
        let FsNode::Directory {
            writeable: Some(dir_host_path),
            ..
        } = parent_node
        else {
            return None;
        };
        Some(dir_host_path.join(&filename))
    }
