...
impl Window {
    ...
    pub fn poll_for_events(&mut self, options: &Options) {
        ...
        while self.enable_event_polling {
            ...
            // Check if Shift is held for pinch-to-zoom simulation
            let shift_held = {
                let kb = self.event_pump.keyboard_state();
                kb.is_scancode_pressed(sdl2::keyboard::Scancode::LShift)
                    || kb.is_scancode_pressed(sdl2::keyboard::Scancode::RShift)
            };
            ...
            self.event_queue.push_back(match event {
                ...
                E::MouseButtonDown {
                    ...
                    // Use mouse position as pinch center; two fingers
                    // start at the same point and spread apart on drag
                    self.pinch_active = true;
                    self.pinch_center = Some(coords);
                    log_dbg!("Pinch begin at {:?}", coords);
                    Event::TouchesDown(HashMap::from([
                        (FingerId::PinchFinger1, coords),
                        (FingerId::PinchFinger2, coords),
                    ]))
                }
                E::MouseButtonDown {
                    x,
                ...
                E::MouseMotion {
                    x, y, mousestate, ..
                } if mousestate.left() && self.pinch_active => {
                    let coords = transform_input_coords(self, (x as f32, y as f32), false);
                    let (cx, cy) = self.pinch_center.unwrap_or(coords);
                    // Mirror the cursor position around the center
                    let finger1 = coords;
                    let finger2 = (2.0 * cx - coords.0, 2.0 * cy - coords.1);
                    log_dbg!("Pinch move f1={:?} f2={:?}", finger1, finger2);
                    Event::TouchesMove(HashMap::from([
                        (FingerId::PinchFinger1, finger1),
                        (FingerId::PinchFinger2, finger2),
                    ]))
                }
                E::MouseMotion {
                    x, y, mousestate, ..
                ...
                E::MouseButtonUp {
                    ...
                    log_dbg!("Pinch end f1={:?} f2={:?}", finger1, finger2);
